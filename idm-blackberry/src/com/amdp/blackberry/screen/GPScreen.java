 ...
 * installation.
 */
public class GPScreen implements ResponseActionDelegate {
	// Constants
	// -----------------------------------------------------------------------------------------------------------------
	private static final int GRADE_INTERVAL = 5; // Seconds - represents the
													// number of updates over
													// which alt is calculated.
	...
	private GPScreen responseActionDelegate = null;
	...
	public GPScreen() {
		...
		screen.setTitle("Por favor espere localizando...");

		_status = new EditField(Field.NON_FOCUSABLE);
		screen.add(_status);

		continueButton = new ButtonField("Reportar ubicación",
		...
		continueButton.setChangeListener(new FieldChangeListener() {
			public void fieldChanged(Field arg0, int arg1) {
				...
				if (latitude != 0) {
					...
					Dialog dialog = new Dialog("No se ha podido determinar su ubicación actual. Por favor espere", new String[] { "Aceptar" }, new int[] { 1 }, 1, Bitmap
	...
	private void reportCurrentLocation() {
		Thread threadToRun = new Thread() {
			public void run() {
				AddCheckinResourceHandler rsourceHandler = new AddCheckinResourceHandler(
						latitude, longitude);
	...
	private void goToNextScreen() {
		...
		if (nextScreen == DIALOG_MESSAGE_SCREEN) {
			AlertCenter
					.getInstance()
					.showAlertDialog(
							"Su ubicación actual no pudo ser enviada. Por favor intente nuevamente");
		} else if (nextScreen == SUCCESSFUL_SCREEN) {
			BrowserSession bSession = Browser.getDefaultSession();
			bSession.displayPage(redirectURL);
	...
	private MenuItem _viewWayPoints = new MenuItem("View waypoints", 110, 11) {
		public void run() {
			GPScreen.this.viewPreviousPoints();
		}
	};
	...
	private void viewOptions() {
		...
		UiApplication.getUiApplication().pushScreen(optionScreen);
	}
	...
	private void viewPreviousPoints() {
		...
		UiApplication.getUiApplication().pushScreen(pointScreen);
	}
	...
	private static double round(double d, int decimal) {
		...
		int d1asint = (int) d1; // Clip the decimal portion away and cache the
								// cast, this is a costly transformation.
	...
	private class LocationListenerImpl implements LocationListener {
		...
		public void locationUpdated(LocationProvider provider, Location location) {
			if (location.isValid()) {
				...
				float grade = (totalDist == 0.0F) ? Float.NaN
						: ((_altitudes[4] - _altitudes[0]) * 100 / totalDist);
				...
				// sb.append("\n");
				// sb.append("Grados : ");
				// if(Float.isNaN(grade))sb.append(" Not disponibles");
				// else sb.append(grade+" %");
				GPScreen.this.updateLocationScreen(sb.toString());
			}
		}
	 ...
	 */
	public final class GPSDemoScreen extends MainScreen {

		// Constructor
		GPSDemoScreen() {
			RichTextField instructions = new RichTextField(
					"Por favor espere...", Field.NON_FOCUSABLE);
	...
	private class OptionScreen extends MainScreen {
		...
		OptionScreen() {
			...
			LabelField title = new LabelField("Options", LabelField.ELLIPSIS
					| LabelField.USE_ALL_WIDTH);
			setTitle(title);

			_serverAddressField = new BasicEditField("Server Address: ",
					GPScreen._hostName, 128, Field.EDITABLE);
		...
		private MenuItem _save = new MenuItem("Save", 200000, 10) {
			public void run() {
				...
				GPScreen._hostName = hostname;
				GPScreen.this._serverConnectThread.setHost(hostname);
				UiApplication.getUiApplication().popScreen(OptionScreen.this);
			}
		};
	}
	...
	static class WayPoint implements Persistable {
		...
		WayPoint(long startTime, long endTime, float distance,
		...
		public void run() {
			...
			while (_go) {
				...
				try {
					...
					GPScreen.this.updateLocationScreen(e.toString());
